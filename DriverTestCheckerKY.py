...
source = requests.get("https://schedule.ky.gov/Booking.Web/Event/Book/338")
...
for medias in (soup.find_all(class_="media")):
        ...
        for potentialtimes in (medias.find_all(class_="row")):
                for individualTimes in (potentialtimes.find_all(class_="instanceSelector")):
                        ...
                        if ((dayofweek == "Monday") or (dayofweek == "Friday")):
...
if os.path.exists("/~/DrivingTest/Jess-Driving-temp.txt"):
        os.remove("/~/DrivingTest/Jess-Driving-temp.txt")
f = open("/~/DrivingTest/Jess-Driving-temp.txt","a")
f.write("\n\n")
f.write("Jessamine County - Driving Test Slots Available!!! \n")
f.write("\n")
f.write("URL to register: https://schedule.ky.gov/Booking.Web/Event/Book/338 \n")
f.write("\n")
...
Recipients = ["RecipientEmailAddress@Test.Com"]

if AreSpotsAvailable == "Yes" and EmailEnabled == "Yes":
        for dest in Recipients:
                f = open("/home/dgarrison/DrivingTest/Jess-Driving-temp.txt","r")
                message = f.read()
                print ("Spots available")
                #email
                s = smtplib.SMTP('smtp.gmail.com', 587)
                s.starttls()
                s.login("SendingAccount@Test.Com", "SendingAPIPassword")
                s.sendmail("SendingAccount@Test.com", dest, message)
                s.quit()
...
g.write("Jessamine Co Driving - Run at: "+ str(datetime.datetime.now()) + "     - Any Spots Available?: " + AreSpotsAvailable + "\n")
g.close()
